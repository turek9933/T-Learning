...
import { useMyEvents } from '@/lib/hooks/event-hooks';
...
export default function CalendarPage() {
    ...
    const { data: events    = [] } = useMyEvents(from, to);
    const { data: homeworks = [] } = useMyHomeworks(from, to);
    ...
    const calendarEvents = useMemo<EventInput[]>(() => [
        ...events.map(ev => {
            ...
            return {
                ...
                extendedProps: {
                    kind:          'event',
                    workspaceSlug: ev.workspaceSlug,
                    originalId:    ev.id,
                    bg:            style.bg,
                    fg:            style.fg,
                },
        ...
        ...homeworks
            ...
            .map(hw => ({
                ...
                extendedProps: {
                    kind:          'homework',
                    workspaceSlug: hw.workspaceSlug,
                    originalId:    hw.id,
                    bg:            HW_STYLE.bg,
                    fg:            HW_STYLE.fg,
                },
    ...
    function handleEventClick(arg: EventClickArg) {
        ...
        if (id.startsWith(HW_PREFIX)) {
            const hw = homeworks.find(h => h.id === id.slice(HW_PREFIX.length));
            if (hw) router.push(`/workspaces/${hw.workspaceSlug}/homeworks/${hw.id}`);
        } else {
            const ev = events.find(e => e.id === id.slice(EV_PREFIX.length));
            if (ev) router.push(`/workspaces/${ev.workspaceSlug}/events/${ev.id}`);
        }
    }
